...
<Accordion title="Click to Learn More About Using Terraform">
  ...
  ```
  Get your AWS Account ID from the CLI with this command:
```
aws sts get-caller-identity \
    --query Account \
    --output text
```
To see the **roles** associated with your account, use this AWS CLI command:
```
aws iam list-roles
```
To see the **policies** associated with your account, use this AWS CLI command:
```
